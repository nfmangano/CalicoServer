...
public class CIntentionClusterGraph {
	...
	static Dimension clusterDimensions;
	...
	private static class Position {
		...
		void centerLayoutInUnitBounds() {
			Dimension clusterDimensions = CIntentionClusterGraph
					.getClusterDimensions();
			Point rootPosition = clusterLayout
					.getLayoutCenterWithinBounds(clusterDimensions);
			// new Dimension(xUnitSpan *
			// CIntentionCluster.CLUSTER_UNIT_SIZE.width, yUnitSpan
			// * CIntentionCluster.CLUSTER_UNIT_SIZE.height));
			int CELL_BUFFER = 20;
			Point center = new Point(
					(int) ((xUnit * (clusterDimensions.width + CELL_BUFFER)) + rootPosition.x),
					(int) ((yUnit * (clusterDimensions.height + CELL_BUFFER)) + rootPosition.y));
			// Point center = new Point((int) ((xUnit *
			// CIntentionCluster.CLUSTER_UNIT_SIZE.width) + rootPosition.x),
			// (int) ((yUnit * CIntentionCluster.CLUSTER_UNIT_SIZE.height) +
			// rootPosition.y));
...
//			}
	...
	private static class UnitGraph {
		...
		void layout(Position position) {
			if (position.isEmpty()) {
				position.xUnitSpan = position.yUnitSpan = 1; // 0 to
																// auto-collapse
																// empty cells

				// Dimension boundingBox =
				// CIntentionClusterGraph.getClusterDimensions();
				position.xUnitSpan = 1;// Math.max(1, (int)
										// Math.ceil(boundingBox.width /
										// CIntentionCluster.CLUSTER_UNIT_SIZE.getWidth()));
				position.yUnitSpan = 1; // Math.max(1, (int)
										// Math.ceil(boundingBox.height /
										// CIntentionCluster.CLUSTER_UNIT_SIZE.getHeight()));
			} else {
				position.layoutInEmptySpace();

				// Dimension boundingBox =
				// position.clusterLayout.getBoundingBox();
				// Dimension boundingBox =
				// CIntentionClusterGraph.getClusterDimensions();
				position.xUnitSpan = 1; // Math.max(1, (int)
										// Math.ceil(boundingBox.width /
...
//					position.normalizeSize(maxClusterDimension);
	...
	/**
	 * Makes all canvases and clusters the same size.
...
//	}
	...
	private Direction getShallowestDirection() {
		...
		for (List<Position> row : graph) {
			maxRightExtent = Math.max(maxRightExtent, row.get(row.size() - 1)
					.getRightExtent());
		}
		...
		return (maxRightExtent > maxDownExtent) ? Direction.DOWN
				: Direction.RIGHT;
	}
	...
	void inflateStoredData(String data) {
		...
		while (rowParser.hasMoreTokens()) {
			...
			while (positionParser.hasMoreTokens()) {
				...
				if (!position.isEmpty()) {
					positionsByRootCanvasId.put(
							position.cluster.getRootCanvasId(), position);
				}
	...
	public int getClusterCount() {
		// private final List<List<Position>> graph = new
		// ArrayList<List<Position>>();
		int counter = 0;
